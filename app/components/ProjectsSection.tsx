...
export function ProjectsSection({ projects, page, onPageChange }: Props) {
  ...
  return <section id="projects">
    ...
    <div className="project-grid">{projects.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE).map((project, index) => {
      ...
      return <a className="project-card" href={project.href} key={project.title} target="_blank" rel="noreferrer">
        <div className={`placeholder tone-${itemNumber}${project.image ? ' project-image' : ''}`} aria-hidden="true">
          {project.image && <img src={project.image} alt="" width="1000" height="690" />}<span>{String(itemNumber).padStart(2, '0')}</span>
        </div>
        <div className="project-copy"><h3>{project.title}</h3><p>{project.description}</p><small>{project.stack}</small></div>
      </a>;
    })}</div>
    <Pagination label="Projects pagination" page={page} pageCount={pageCount} onChange={onPageChange} />
  </section>;
}
